  ...
  useEffect(() => {
    ...
    const updateVal = (e, val, i) => {
      ...
      if (productValue <= 4) {
        ...
        firebase.database().ref(`/addCarts/${uid}`).child(keys[i]).update({totalPrice: ans})
        firebase.database().ref(`/addCarts/${uid}`).child(keys[i]).update({productValue: productValue})
    }
    }
            ...
            >
              <div>
                ...
                <div style={{ display: "flex" }}>
                  ...
                  <div onClick={(e) => updateVal(e, val, index)} style={{backgroundColor: 'green', width: 60, height: 60, borderRadius: 30, display: 'flex', justifyContent: 'center', alignItems: 'center', cursor: 'pointer', marginLeft: 20}}>
                  ...
                  <div style={{display: 'flex', justifyContent: 'center', alignItems: 'center', marginLeft: 15, backgroundColor: '#f36e36', width: 40, height: 40, borderRadius: 40}}>
                    <span style={{fontSize: 30, fontStyle: 'revert', color: '#f1f1f1'}}>{val.productValue}</span>
                  </div>
                  </div>

                  <div onClick={(e) => deleteVal(e, val, index)} style={{backgroundColor: 'red', width: 60, height: 60, borderRadius: 30, display: 'flex', justifyContent: 'center', alignItems: 'center', cursor: 'pointer', marginLeft: 20}}>
                  <Icon.CartDashFill size={30} style={{color: '#f1f1f1'}} />
                    ...
                    <p style={{ marginLeft: 15, fontSize: 25, paddingTop: 10}}><b style={{color: '#f3f3f3'}}>Total Amount :</b>  <span style={{color: 'blue' }}>${val.totalPrice}</span> </p>
                    </div>
                      </div>

                    <div style={{display: 'flex', justifyContent: 'space-evenly', marginRight: 40}}>
                    <div className="d-grid gap-2" onClick={(e) => goToCheckOutForm(e, val, index)}>
                    ...
                    <button style={{borderRadius: 12,}} className="btn btn-danger" onClick={(e) => deletedCart(e, index)}>
